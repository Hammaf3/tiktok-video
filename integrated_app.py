...
DownloadError = None  # Will be set if enhanced downloader available
...
try:
    ...
    print(f"⚠️  Enhanced downloader not available, trying simple: {e}")
    try:
        ...
        print(f"⚠️  Simple downloader not available, trying original: {e}")
        try:
            ...
            YT2TIK_AVAILABLE = True
            DownloadError = Exception  # Fallback
            print("⚠️  Using original downloader")
        except ImportError as e:
...
# Use /tmp on Railway for ephemeral but writable storage
    ...
    try:
        ...
        health_status = {
            ...
            'components': {
                ...
                'enhanced_downloader': DownloadError is not None and DownloadError != Exception,
            ...
            'job_stats': job_store.get_stats() if hasattr(job_store, 'get_stats') else {'info': 'stats not available'},
            'system': {
...
def convert():
    """Convert YouTube video with comprehensive validation and logging"""
    try:
        print(f"\n[CONVERT] ========== New conversion request ==========")

        # Validate request data
        if not request.json:
            print(f"[CONVERT] Error: No JSON data in request")
            return jsonify({'error': 'Invalid request - JSON data required'}), 400
        ...
        print(f"[CONVERT] Duration: {duration}s, Start time: {start_time if start_time else 'auto'}")
        print(f"[CONVERT] Auto-detect: {auto_detect}, Auto-upload: {auto_upload}")

        # Validate YouTube URL
        if not youtube_url:
            print(f"[CONVERT] Error: Empty URL")
            return jsonify({'error': 'YouTube URL is required'}), 400

        # Basic URL validation
        if not ('youtube.com' in youtube_url or 'youtu.be' in youtube_url):
            print(f"[CONVERT] Error: Invalid URL format")
        ...
        try:
            ...
            if duration < 5:
                print(f"[CONVERT] Error: Duration too short ({duration}s)")
                return jsonify({'error': 'Duration must be at least 5 seconds'}), 400
            if duration > 180:
                print(f"[CONVERT] Error: Duration too long ({duration}s)")
                return jsonify({'error': 'Duration cannot exceed 180 seconds (3 minutes)'}), 400
        except (ValueError, TypeError):
            print(f"[CONVERT] Error: Invalid duration value")
        ...
        if start_time:
            try:
                ...
                print(f"[CONVERT] Error: Invalid start time format: {start_time}")
        ...
        if auto_upload and not session.get('tiktok_connected'):
            print(f"[CONVERT] Error: TikTok not connected for auto-upload")
            return jsonify({'error': 'Please connect your TikTok account first to enable auto-upload'}), 400

        print(f"[CONVERT] ✅ Validation passed")
        ...
        print(f"[CONVERT] Starting background thread for job {job_id}")
...
def get_status(job_id):
    """Get conversion status - never returns 404"""
    try:
        if not job_id:
            return jsonify({
                'success': False,
        ...
        if not job_data:
            # Return 200 with status info instead of 404
            return jsonify({
                'job_id': job_id,
                'status': 'not_found',
                'success': False,
...
def process_video_safe(job_id: str, youtube_url: str, start_time: str,
    ...
    """
    print(f"[PROCESS] Starting safe processing for job {job_id}")
    try:
        process_video(job_id, youtube_url, start_time, duration, caption, auto_detect, auto_upload)
        print(f"[PROCESS] ✅ Completed successfully for job {job_id}")
    except Exception as e:
        error_msg = str(e)
        print(f"[PROCESS] ❌ FATAL ERROR in job {job_id}: {error_msg}")
        ...
        try:
            ...
            print(f"[PROCESS] Job {job_id} marked as error")
        except Exception as update_error:
            print(f"[PROCESS] Failed to update job status: {str(update_error)}")
...
def process_video(job_id, youtube_url, start_time, duration, caption, auto_detect, auto_upload=False):
    ...
    try:
        safe_print(f"\n[VIDEO] ========================================")
        safe_print(f"[VIDEO] Processing job {job_id}")
        safe_print(f"[VIDEO] URL: {youtube_url}")
        safe_print(f"[VIDEO] Duration: {duration}s, Start: {start_time if start_time else 'beginning'}")
        safe_print(f"[VIDEO] ========================================")

        # Step 1: Download video
        update_job_status('processing', 10, 'Starting download...')
        safe_print(f"[VIDEO] Step 1/3: Download")

        try:
            update_job_status('processing', 20, 'Downloading YouTube video...')
            safe_print(f"[VIDEO] Calling download_youtube_video...")
            video_data = download_youtube_video(youtube_url)

            if not video_data or 'video_path' not in video_data:
                safe_print(f"[VIDEO] ❌ Download returned no data")
                raise Exception("Download failed - no video data returned")

            video_path = video_data['video_path']
            video_title = video_data.get('title', 'Unknown')

            safe_print(f"[VIDEO] ✅ Download successful")
            safe_print(f"[VIDEO] Title: {video_title}")
            safe_print(f"[VIDEO] Path: {video_path}")

            # Verify downloaded file exists
            if not Path(video_path).exists():
                safe_print(f"[VIDEO] ❌ Downloaded file not found at path")
                raise Exception("Download failed - video file not found")

            file_size_mb = Path(video_path).stat().st_size / (1024 * 1024)
            safe_print(f"[VIDEO] File size: {file_size_mb:.1f} MB")

        except Exception as e:
            error_msg = str(e)
            safe_print(f"[VIDEO] ❌ Download error: {error_msg}")

            # Enhanced error handling with specific error codes
            if DownloadError and isinstance(e, DownloadError):
                error_code = getattr(e, 'error_code', 'DOWNLOAD_FAILED')
                safe_print(f"[VIDEO] Error code: {error_code}")

                # Map error codes to user-friendly messages
                error_messages = {
                    'HTTP_429': 'YouTube is rate limiting requests. Please wait a few minutes and try again.',
                    'LOGIN_REQUIRED': 'This video requires YouTube authentication or has bot protection enabled. Please try a different video.',
                    'PRIVATE_VIDEO': 'This video is private and cannot be accessed.',
                    'VIDEO_UNAVAILABLE': 'This video is unavailable, deleted, or region-locked.',
                    'COPYRIGHT_CLAIM': 'This video has been removed due to a copyright claim.',
                    'AGE_RESTRICTED': 'This video is age-restricted and requires authentication.',
                    'LIVE_STREAM': 'Cannot download live streams. Please wait until the stream ends.',
                    'MEMBERSHIP_REQUIRED': 'This video requires channel membership or payment to access.',
                    'NETWORK_ERROR': 'Network connection error. Please check your internet connection and try again.',
                }

                user_message = error_messages.get(error_code, f'Download failed: {error_msg}')
                update_job_status('error', 0, user_message)
            else:
                # Fallback for older error format
                if error_msg.startswith('RESTRICTED:'):
                    safe_print(f"[VIDEO] Error type: RESTRICTED")
                    ...
                    safe_print(f"[VIDEO] Error type: UNAVAILABLE")
                    update_job_status('error', 0,
                        'Video is unavailable, deleted, or region-locked.')
                elif error_msg.startswith('COPYRIGHT:'):
                    safe_print(f"[VIDEO] Error type: COPYRIGHT")
                    update_job_status('error', 0,
                        'Video removed due to copyright claim.')
                elif error_msg.startswith('LIVE_STREAM:'):
                    safe_print(f"[VIDEO] Error type: LIVE_STREAM")
                    update_job_status('error', 0,
                        'Cannot download live streams. Please use a regular video.')
                else:
                    # Generic error - clean up the message
                    clean_msg = error_msg.replace('DOWNLOAD_FAILED: ', '')
                    safe_print(f"[VIDEO] Error type: GENERIC")
                    update_job_status('error', 0, f'Download failed: {clean_msg}')
            return

        # Step 2: Convert video
        safe_print(f"[VIDEO] Step 2/3: Convert to TikTok format")
        try:
            ...
            safe_print(f"[VIDEO] Output filename: {output_filename}")
            ...
            if not Path(converted_path).exists():
                safe_print(f"[VIDEO] ❌ Conversion output file not created")
                raise Exception("Conversion failed - output file not created")

            output_size_mb = Path(converted_path).stat().st_size / (1024 * 1024)
            safe_print(f"[VIDEO] ✅ Conversion successful: {output_size_mb:.1f} MB")

        except Exception as e:
            error_msg = str(e)
            safe_print(f"[VIDEO] ❌ Conversion error: {error_msg}")
        ...
        safe_print(f"[VIDEO] Step 3/3: Finalize and store results")
        try:
            ...
            safe_print(f"[VIDEO] ✅ Job {job_id} completed successfully")
            safe_print(f"[VIDEO] Download URL: /download/{output_filename}")
            safe_print(f"[VIDEO] ========================================\n")

        except Exception as e:
            safe_print(f"[VIDEO] ❌ Error storing results: {str(e)}")
        ...
        error_msg = str(e)
        safe_print(f"[VIDEO] ❌ UNEXPECTED ERROR: {error_msg}")

        try:
            import traceback
            safe_print(f"[VIDEO] Stack trace:")
            traceback.print_exc()
        except:
            pass
